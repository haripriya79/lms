...
if($query->rowCount() > 0)
{
	...
	$emp_email = htmlentities($result->EmailId);
...
if(!empty($emp_email) && !empty($emp_name)){
...
$mail->setFrom($mail->Username, 'JNTUA LMS');
$mail->addReplyTo($mail->Username, 'JNTUA LMS');
$mail->addAddress($emp_email, $emp_name);
$mail->Subject = 'JNTUA LEAVES - INFORMATION - Reg ';
if($status==1){
	$mail->Body = "Dear ".$emp_name.",<br /><br />Your Leave request is <b>Approved</b>.<br /><br />Thanks and regards"; 
	$mail->AltBody = "Dear ".$emp_name.",\n\nYour Leave request is Approved.\n\nThanks and regards"; 
}elseif($status==2){

    
	$mail->Body = "Dear ".$emp_name.",<br /><br />Your Leave request is <b>Not Approved</b> <br /><br />Thanks and regards"; 
	$mail->AltBody = "Dear ".$emp_name.",\n\nYour Leave request is Not Approved.\n\nThanks and regards"; 
...
<?php 
$lid=intval($_GET['leaveid']);
$sql = "SELECT tblleaves.id as lid,tblemployees.FirstName,tblemployees.LastName,tblemployees.EmpId,tblemployees.id,tblemployees.Gender,tblemployees.Phonenumber,tblemployees.EmailId,tblleaves.LeaveType,tblleaves.TotalDays,tblleaves.ToDate,tblleaves.FromDate,tblleaves.Description,tblleaves.PostingDate,tblleaves.Status,tblleaves.AdminRemark,tblleaves.AdminRemarkDate from tblleaves join tblemployees on tblleaves.empid=tblemployees.id where tblleaves.id=:lid";
$query = $dbh -> prepare($sql);
$query->bindParam(':lid',$lid,PDO::PARAM_STR);
$query->execute();
